Feature: Add Card Modal availability toggle and reactive filtering
  ...
  @delta-added
  Scenario: "Solo disponibles" toggle filters to available cards when active
    Given the Add Card modal is open for abilities
    And the "Solo disponibles" toggle is enabled
  ...
  @delta-added
  Scenario: "Solo disponibles" toggle shows all cards when deactivated
    Given the Add Card modal is open for abilities
    And the "Solo disponibles" toggle is enabled
    When the user disables the "Solo disponibles" toggle
