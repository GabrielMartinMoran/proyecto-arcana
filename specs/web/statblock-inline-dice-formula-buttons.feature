...
Feature: Web statblock inline dice formula buttons
  ...
  Inline formula button visible and accessibility names include the dice icon, while clicked roll expressions remain formula-only.
  Inline formula roll titles use the format "<creature name>: <source name>".
  Behavior text is rendered as Markdown-visible text only and is not an inline dice roll source.
  ...
  Scenario: Render a dice formula in an action detail as a button
    ...
    Then the action detail shows a button with visible and accessibility name "1d6 🎲"
  ...
  Scenario: Render multiple formulas in one ability detail
    ...
    When the user views the statblock actions
    Then the action detail shows a button with visible and accessibility name "1d6 + 5 + 4d2 🎲"
    And the action detail shows a button with visible and accessibility name "2d6 + 3 🎲"
  ...
  Scenario: Click an inline formula button to roll with creature-prefixed source title
    ...
    When the user clicks the inline formula button with visible and accessibility name "1d8+1 🎲"
  ...
  @attack-notes
  Scenario: Render a formula in an attack note
    Given an attack named "Mordida" has note "El objetivo sangra 1d6 + 2"
    When the user views the statblock attacks
    Then the attack note shows a button with visible and accessibility name "1d6 + 2 🎲"

  @attack-notes @rolling
  Scenario: Roll a formula from an attack note with creature-prefixed source title
    Given a creature named "Bestia ígnea"
    And an attack named "Mordida" has note "El objetivo sangra 1d6 + 2"
    When the user clicks the inline formula button with visible and accessibility name "1d6 + 2 🎲"
  ...
  Scenario Outline: Render inline formula buttons in requested statblock fields
    ...
    Then the <field> detail shows a button with visible and accessibility name "<button>"
  ...
  @behavior @stat-notes
  Scenario: Render Markdown-only behavior text and inline formula buttons in stat notes
    Given the creature behavior contains "Prefiere usar **1d6** trucos"
    And the creature speed note contains "Corre 2d6 metros extra"
    When the user views the statblock
    Then the behavior text shows visible Markdown text "Prefiere usar 1d6 trucos"
    And the behavior text shows no inline formula buttons
    And the speed note shows a button with visible and accessibility name "2d6 🎲"

  @attack-notes @stat-notes @rolling
  ...
  Scenario: Preserve XSS safety while rendering inline formula buttons
    ...
    And the trait detail shows a button with visible and accessibility name "1d6 🎲"
  ...
  Scenario: Inline formula rolls reuse the direct roll service in Foundry embedded mode
    ...
    When the user clicks the inline formula button with visible and accessibility name "2d6 🎲"
