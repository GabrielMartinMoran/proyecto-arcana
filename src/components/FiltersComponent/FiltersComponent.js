...
const FiltersComponent = (container, props = {}) => {
    let state = {
        facets: props.facets || { attributes: [], tags: [], types: ["Accionable","De Efecto"], levels: [1,2,3,4,5] },
    ...
    const bindEvents = () => {
        ...
        if (clear) clear.addEventListener('click', () => { state.value = { text: '', levels: [], types: [], attributes: [], tags: [] }; state.onClear(); });
        if (tagsToggle) tagsToggle.addEventListener('click', () => { state.tagsOpen = !state.tagsOpen; setState({}); });
        if (tagsClose) tagsClose.addEventListener('click', () => { state.tagsOpen = false; setState({}); });
        if (tagsClear) tagsClear.addEventListener('click', () => { state.value.tags = []; setState({}); state.onChange(state.value); });
        ...
        tagChecks.forEach(ch => ch.addEventListener('change', (e) => {
            const v = e.target.value;
