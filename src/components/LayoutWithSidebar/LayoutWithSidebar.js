...
const LayoutWithSidebar = (container, props = {}) => {
    ...
    const loadStyles = () => {
        ensureStyle('./src/components/LayoutWithSidebar/LayoutWithSidebar.css');
    };
    ...
    return {
        ...
        getMainEl() {
            return container.querySelector('.layout-slot');
        },
        setSidebarExtra(title, html) {
            lastExtra = { title: title || 'Indice', html: html || '' };
            if (sidebarApi) sidebarApi.setExtra(lastExtra.title, lastExtra.html);
        },
    };
};
