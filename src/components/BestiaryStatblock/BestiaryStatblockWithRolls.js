                  ...
                  .map(
                      (a) =>
                          html`<li>
                              ...
                              <button class="button small" data-roll-attack="${escapeHtml(a.name)}" data-attack-bonus="${Number(a.bonus) || 0}" data-attack-damage="${escapeHtml(a.damage)}" title="Tirar ataque">🎲</button>
                          </li>`
                  )
                  .join('')}
          </ul>`
    ...
    return html` <article class="statblock" id="${slugify(c.name)}">
        ...
        <div class="attributes">
            <span class="kv">
                ...
                <button class="button small" data-roll-attribute="cuerpo" data-attribute-value="${c.attributes?.cuerpo || 0}" title="Tirar Cuerpo">🎲</button>
            </span>
            <span class="kv">
                <strong>Reflejos:</strong> ${escapeHtml(c.attributes?.reflejos)}
                <button class="button small" data-roll-attribute="reflejos" data-attribute-value="${c.attributes?.reflejos || 0}" title="Tirar Reflejos">🎲</button>
            </span>
            <span class="kv">
                <strong>Mente:</strong> ${escapeHtml(c.attributes?.mente)}
                <button class="button small" data-roll-attribute="mente" data-attribute-value="${c.attributes?.mente || 0}" title="Tirar Mente">🎲</button>
            </span>
            <span class="kv">
                <strong>Instinto:</strong> ${escapeHtml(c.attributes?.instinto)}
                <button class="button small" data-roll-attribute="instinto" data-attribute-value="${c.attributes?.instinto || 0}" title="Tirar Instinto">🎲</button>
            </span>
            <span class="kv">
                <strong>Presencia:</strong> ${escapeHtml(c.attributes?.presencia)}
                <button class="button small" data-roll-attribute="presencia" data-attribute-value="${c.attributes?.presencia || 0}" title="Tirar Presencia">🎲</button>
            </span>
        </div>
        ...
        ${c.behavior
            ...
            : ''}
        ${c.description ? html`<div class="section"><strong>Descripción:</strong> ${escapeHtml(c.description)}</div>` : ''}
    </article>`;
}

export function bindBestiaryRollEvents(container, creature, activeParticipant = null, selectedCreatureIndex = null, onPersist = null) {
    ...
    container.querySelectorAll('[data-roll-attribute]').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            openRollModal(container, {
                ...
                const characterName = activeParticipant && activeParticipant.type === 'npc-group' && selectedCreatureIndex !== null
        ...
        btn.addEventListener('click', () => {
            ...
            openRollModal(container, {
                ...
                if (attackDamage && attackDamage.trim()) {
                    setTimeout(() => {
                        rollDamage(container, characterName, attackName, attackDamage, activeParticipant, selectedCreatureIndex, onPersist);
...
function rollDamage(container, creatureName, attackName, damageFormula, activeParticipant = null, selectedCreatureIndex = null, onPersist = null) {
