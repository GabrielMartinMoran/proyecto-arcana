...
const CharacterEditor = (container, props = {}) => {
    ...
    const render = () => {
        ...
        const header = html`<div class="editor-header">
        ...
        const sheet = html`<div class="editor-grid">
            ...
            <div class="panel">
                ...
                <div class="attrs">
                    <div class="attr">
                        <span>PP</span><input type="number" id="pp" min="0" step="1" value="${c.pp || 0}" />
                    </div>
                    <div class="attr">
                        <span>Suerte</span>
                        <div class="hp-wrap">
            ...
            <div class="panel">
                ...
                <div class="attrs">
                    <div class="attr">
                        <span>Oro</span
                        ><input class="long-input" type="number" id="gold" min="0" step="1" value="${c.gold || 0}" />
                    </div>
                </div>
            </div>
            <div class="panel">
                <label>Equipo</label><textarea id="equipment" rows="6">${c.equipment || ''}</textarea>
            </div>
        ...
        const cards = html`<div class="editor-grid one-col">
            ...
            <div class="panel">
                ...
                <div class="cards-grid">
                    ${(c.activeCards || [])
                        ...
                        .map(
                            (card) => html`<div class="card-slot" data-id="${card.id}" data-actions="deactivate"></div>`
                        )
                        .join('')}
                </div>
            </div>
            ...
            <div class="panel">
                ...
                <div class="cards-grid">
                    ${CardService.filter(state.allCards, state.filters)
            ...
            const actionsRenderer = (c) => {
                ...
                if (mode === 'toggle') {
                    ...
                    const toggleBtn = canActivate
                        ? `<button class="button" data-toggle-active="${c.id}">${active ? 'Desactivar' : 'Activar'}</button>`
    ...
    const bind = () => {
        ...
        const tempHp = container.querySelector('#temp-hp');
        const pp = container.querySelector('#pp');
        const suerte = container.querySelector('#suerte');
        const gold = container.querySelector('#gold');
        const equipment = container.querySelector('#equipment');
        const cardSearch = container.querySelector('#card-search');
        const activeSlots = container.querySelector('#active-slots');
        if (hp)
        ...
        container.querySelectorAll('[data-roll-attr]').forEach((btn) =>
            btn.addEventListener('click', () => {
                ...
                const val = Number(state.character.attributes[key]) || 0;
                const derived = compute(state.character);
                openRollModal(
                    document.body,
                    { attributeName: key, attributeValue: val, maxSuerte: Number(derived.suerteMax) || 0 },
                    (res) => {
                        if (res && res.luck) {
                            state.character.suerte = Math.max(0, (state.character.suerte || 0) - res.luck);
                            state.onChange(state.character);
                            setState({});
                        }
                    }
                );
