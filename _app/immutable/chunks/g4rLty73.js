import{m as r}from"./DZAtAn7e.js";import{l as m}from"./BQL_5efD.js";import{m as p,a as g}from"./DpY7wP4R.js";import{r as d}from"./BmoKZM79.js";const o=async s=>await(await fetch(d(s))).text(),y=s=>{let t="";return t+=`# ${s.name}
...
`}),t+=`
`),t},e=s=>{switch(s?.type){case"RELOAD":return`${s.qty} [Recarga ${s.qty}+]`;case"USES":return`${s.qty}`;case"LONG_REST":case"DAY":return`${s.qty} por día`;default:return""}},A="/docs/bestiary.yml",T=async()=>{const s=await o(A);let t=[];try{t=m(s).creatures??[]}catch(n){console.error("Error parsing YAML:",n)}return t.map(n=>r(n)).map(y).join(`---
...
| --- | --- | --- | --- | --- | --- | --- |
${a.join(`
`)}`},$=async(s,t,a)=>{const n=await o(s);let l=[];try{l=m(n)[a]??[]}catch(i){console.error("Error parsing YAML:",i)}const c=l.map(i=>t(i));return u(c)},I=async()=>await $(h,p,"cards"),v=async()=>await $(f,g,"items"),L="/docs/gm.md",S=async()=>await o(L),b="/docs/player.md",q=async()=>await o(b);export{q as a,S as b,T as c,I as d,v as e,o as l};
