import{m as $}from"./DZAtAn7e.js";import{l as m}from"./BQL_5efD.js";import{m as p,a as d}from"./DpY7wP4R.js";import{r as g}from"./DCWF3LZ_.js";const o=async s=>await(await fetch(g(s))).text(),y=s=>{let t="";return t+=`# ${s.name}
...
`}),t+=`
`),t},e=s=>{switch(s?.type){case"RELOAD":return`${s.qty} [Recarga ${s.qty}+]`;case"USES":return`${s.qty}`;case"LONG_REST":case"DAY":return`${s.qty} por día`;default:return""}},A="/docs/bestiary.yml",T=async()=>{const s=await o(A);let t=[];try{t=m(s).creatures??[]}catch(n){console.error("Error parsing YAML:",n)}return t.map(n=>$(n)).map(y).join(`---

`)},h="/docs/cards.yml",f="/docs/magical-items.yml",M=s=>{if(!s||!s.type)return"N/A";switch(s.type){case"LONG_REST":return`${s.qty??"—"} por día de descanso`;case"RELOAD":return`1 (Recarga ${s.qty??"—"}+)`;case"USES":return`${s.qty??"—"}`;case"DAY":return"1 por día";default:return"—"}},E=s=>{const t=[];return t.push(s.name),t.push(s.level.toString()),t.push(s.type),t.push(s.description),t.push(s.tags?s.tags.join(", "):"—"),t.push(s.requirements??"—"),t.push(M(s.uses)),s.cardType==="item"&&t.push(s.cost.toString()),`| ${t.map(a=>a.replace(/\|/g,"\\|")).join(" | ")} |`},u=s=>{const t=["Nombre","Nivel","Tipo","Descripción","Etiquetas","Requerimientos","Usos"];s[0].cardType==="item"&&t.push("Costo (oro)");const a=s.map(E);return`| **${t.join("** | **")}** |
| --- | --- | --- | --- | --- | --- | --- |
${a.join(`
`)}`},r=async(s,t,a)=>{const n=await o(s);let l=[];try{l=m(n)[a]??[]}catch(i){console.error("Error parsing YAML:",i)}const c=l.map(i=>t(i));return u(c)},I=async()=>await r(h,p,"cards"),v=async()=>await r(f,d,"items"),b="/docs/gm.md",S=async()=>await o(b),L="/docs/player.md",q=async()=>await o(L);export{q as a,S as b,T as c,I as d,v as e,o as l};
